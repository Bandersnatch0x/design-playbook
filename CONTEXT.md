...
| **Tiered run profile** | Every run is graded P1 point-fix / P2 standard / P3 full by declaration-touch surface (which decided fields, criteria, or E-tier design triggers it revises); tier is agent-proposed and user-confirmed once per run, promotion is automatic while demotion is user-only, and process weight scales with change consequence. The tier lives in a mandatory `run-profile` block at the top of `plan.md`; structural gates (G1-G12) are never waivable — only rule hits carry exemptions | uniform-weight runs for one-line fixes; skipping the tier block; waiving structural gates |
