...
任务级流水线：路由 → 探路 → 拷问 → 成谱 → 成票 → 落码 → 验收，外加两个横切机制（按需触发的检验追问、决策点的圆桌辩论）与三条粘性规则（点名按职责延续、裁决批次、自研措辞）。驱动命令 `/dev-next`（`.claude/commands/`）。See `.agents/dev-workflow.md`（本地，不入库；触发条件与跨阶段适用性以该文件为准）.
