...
- [ ] **1. Plugin loads:** `claude --plugin-dir <abs>/packages/design-playbook` starts; `/reload-plugins` reports no errors; eight skills + three commands appear under the `design-playbook` namespace in `/help`. **Semi-automated (v0.4):** `scripts/doctor.py` checks the static counts (8 skills / 3 commands / plugin.json namespace); the dynamic `--plugin-dir` load + `/help` listing stay human (host slash, not automatable).
