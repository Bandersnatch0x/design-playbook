...
- **档位是记录与确认义务的分级，不是决策权的再分配**：C 档仍是代理自主；任何时点用户可把任何决定升级为亲自裁决（Q7 是默认表，用户保留全权）。（落码注：G10 按 4.1 schema 裁定——R/C 档 `confirmation.kind` 恒为 `agent`，「升级为亲自裁决」的实现路径是把该决定**重新定档到 E** 后走用户确认，而不是在 R/C 条目上记 user-kind；R/C 条目记 user 触发 `G10.rc_confirmation_not_agent`。）
