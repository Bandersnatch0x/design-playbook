...
| `execution.preview` / `source.preview` | Preview integrity owner | `RunFacts.preview` from `mcp.preview.integrity.inspect_preview`; never `run_status` narration | `authority-record` / `authority-record-excerpt` | No occurrence is known `absent`; valid current confirm is `confirmed`; aborted/open/invalid follow owner facts; unreadable/malformed/hash mismatch cannot be upgraded to confirmed | `mcp/preview/test_integrity.py`, fixtures `g5-preview-confirmed`, `g5-only-aborted`, `g5-preview-without-confirm` | State/round equal one `PreviewSnapshot`; hash mismatch remains invalid/non-known according to the owner adapter |
...
4. Rebuilding the same immutable set is byte-equivalent after only the three
   approved volatile treatments. Changed or partial sources never reuse the
