...
The preview control surface, the evidence static-handoff page, and the Run
Console share one locale module (`zh-CN` / `en`; `DPB_PREVIEW_LANG` or
`LANG` selects, explicit values win). **Keyboard change:** in the preview
review surface a bare `Esc` now only resets the active tool — submitting a
skip requires `Shift+Esc`, and the inline hint labels the shifted chord.
Operators carrying the pre-v0.22 muscle memory will notice. On Windows the
`open-console` continuation command is PowerShell syntax (single-quoted
literals behind the `&` call operator) — paste it into PowerShell, not
cmd.exe.
