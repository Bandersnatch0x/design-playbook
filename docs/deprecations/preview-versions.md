...
`compatibility.py` owns these reads and the version section of `log.md`.
`transaction.py` supplies the durable decision projection; when no
`version-*.json` files exist, the compatibility projection returns it
unchanged.

## Production dependency migrated

`packages/design-playbook/mcp/preview/transaction.py`, in
`_commit_projections_unlocked`, now imports `render_versions_log` from
`compatibility.py`. Production no longer depends on `versions.py`.

## Long-lived owner for compatibility reading and log projection

The long-lived owner is
`packages/design-playbook/mcp/preview/compatibility.py`. The Preview decision
transaction module retains durable decision authority
(`decision-round-*.json`, confirm records, and the base `log.md` projection)
and supplies it to compatibility reads through a read-only interface.

The following have moved into the long-lived owner:

- `render_versions_log` (log projection incl. the versions section);
- version parsing, `state_at`, `timeline`, and `list_versions`, so historical
  artifacts remain readable after `versions.py` is deleted.
...
1. **Migrate log projection.** Complete: `compatibility.py` owns the version
   section and `transaction.py` imports it directly.
2. **Migrate compatibility reading.** Complete: historical version parsing,
   state replay, timeline, and listing live in `compatibility.py`.
...
4. **Drop the production import.** Complete: the freeze guard proves
   `transaction.py` imports `compatibility.py`, not `versions.py`.
