...
[![Version](https://img.shields.io/badge/Version-0.4.4-2DD4BF?style=flat-square&logo=semver&logoColor=black)](.#)
...
Claude Code / Codex 插件。每次跑同一条可预测链路 — **Design I/O**：`reference-intake? → ux-spec? → plan? → (native-craft?) → ui-picker → (preview*) → fill → craft-guard → (observe*) → ui-evaluator`，验收把每个问题**指回**所属声明，blocking 必须**回流**直到闭环。`?` 为条件入场（有截图/URL/设计稿/产品类比时跑 `reference-intake?`）；`preview*`/`observe*` 仅在对应可选 MCP 工具（`preview_prototype` / `execute_capture_plan`）存在时运行，否则跳过直进下一阶段。
...
七个 model 触发 skill（`/design-playbook:<名>`）：
