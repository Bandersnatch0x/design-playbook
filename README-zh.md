...
orchestrator 会**探测**这两个适配器，缺席即跳过对应步骤——都不是依赖。

| 适配器 | MCP 工具 | 启用 | 安装 |
| :--- | :--- | :--- | :--- |
| `design-playbook-preview` | `preview_prototype` | `preview*` 人工确认门（G5） | [Install / MCP config](./packages/design-playbook-preview/#install--mcp-config) |
| `design-playbook-evidence` | `execute_capture_plan` | `observe*` 运行时取证（G6）——需 Playwright + Chromium | [Install / MCP config](./packages/design-playbook-evidence/#install--mcp-config) |
...
packages/design-playbook-preview/ ← 可选 MCP：preview_prototype（G5 / preview*）
packages/design-playbook-evidence/← 可选 MCP：execute_capture_plan（G6 / observe*）
