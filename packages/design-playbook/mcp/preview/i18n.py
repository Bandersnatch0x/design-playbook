"""i18n for the preview adapter UI (ADR: CJK-first, en supported).

Locale source: env DPB_PREVIEW_LANG (or LANG), default "zh-CN". Only zh-CN and
en are provided; unknown locales fall back to zh-CN.

CONFIRM/revise labels are locale-aware: DEFAULT_OPTIONS returns the options in
the active locale, and CONFIRM_LABELS is the union across locales so the
backend still recognises a confirm regardless of UI language.
"""
from __future__ import annotations
from design_playbook.mcp.ui_locale import EN, ZH, resolve_ui_locale

_STRINGS: dict[str, dict[str, str]] = {
    ZH: {
        "round_n": "第 {n} 轮",
        "annotate": "批注",
        "ready": "可确认",
        "drawer_title": "批注与确认",
        "drawer_empty_title": "还没有批注",
        "drawer_empty_desc": "在左侧画布直接点选元素添加行内意见，按 B 框选区域，或按 D 圈画问题；整体意见写在下方",
        "collapse": "收起",
        "duplicate_anchor": "该元素已是批注 {n}，未重复添加",
        "coachmark_title": "开始标注",
        "coachmark_text": "点击画布任意元素添加批注，Ctrl+↵ 确认通过（按 ? 查看快捷键）",
        "onboard_close": "知道了",
        "anchor_num_pre": "锚点 ",
        "anchor_num_post": " 的修改意见",
        "anchor_placeholder": "这条要改什么",
        "remove_num_pre": "移除锚点 ",
        "field_label": "整体意见（可选）",
        "field_hint": "请先在画布点选元素加批注，或在下方写整体意见",
        "field_placeholder": "对整页架构、风格的总体评价；局部问题请直接在画布点选标注",
        "close": "关闭",
        "cancel": "取消",
        "terminate": "终止评审",
        "terminate_confirm": "确认终止本次预览？",
        "terminate_confirm_go": "确认终止",
        "terminate_desc": "终止整次预览会话（不是通过/修改）；需在弹出层再次确认",
        "abort_cancelled": "已取消终止",
        "abort_popover_aria": "确认终止预览",
        "draft": "保留批注，暂不决定",
        "draft_desc": "关闭抽屉并保留批注，不提交本轮决定",
        "remove": "移除",
        "anchor_note_label": "锚点批注 ({n}):",
        "no_text": "(无文字)",
        "done_title": "已记录",
        "done_body": "窗口即将自动关闭。",
        "confirm": "确认通过",
        "skip": "跳过",
        "skip_desc": "跳过本轮人工评审，不代表确认通过（Shift+Esc）",
        "zoom_fit": "自适应",
        "draw_toggle": "圈画标注",
        "draw_on": "圈画中 · 再点关闭",
        "draw_label": "圈画 {n}",
        "tool_box": "区域高亮框",
        "tool_ruler": "红线标尺与间距探针",
        "box_label": "区域 {n}",
        "ruler_size": "{w}×{h} px",
        "ruler_distance": "{d} px",
        "app_title": "预览确认",
        "mode_preview": "预览",
        "mode_annotate": "批注",
        "vp_desktop": "桌面视图 [1024px]",
        "vp_tablet": "平板视图 [768px]",
        "vp_mobile": "手机视图 [375px]",
        "filter_all": "全部",
        "filter_pending": "待处理",
        "filter_resolved": "已解决",
        "mark_resolved": "标记为已解决",
        "reopen": "重新打开",
        "tag_copy": "文案",
        "tag_layout": "布局",
        "tag_visual": "视觉",
        "tag_draw": "圈画",
        "tag_box": "框选",
        "tag_pin": "定位点",
        "tag_note": "批注",
        "tag_element": "元素",
        "popover_placeholder": "输入修改意见 (Enter 保存 · Esc 取消)",
        "popover_save": "保存",
        "popover_aria": "行内批注输入",
        "tab_annotations": "批注清单",
        "tab_spec": "验收准则",
        "approve_ready": "确认通过 ({n} 处批注)",
        "approve_ready_one": "确认通过 ({n} 处批注)",
        "approve_not_ready": "写意见或点选标注后确认",
        "status_ready": "已定位 {n} 处批注 · 可确认通过",
        "status_not_ready": "写意见或添加批注后再确认",
        "shortcuts_title": "快捷键指南",
        "group_global": "全局决策流转",
        "group_tools": "画布与交互工具",
        "lang_toggle": "切换中英双语",
        "lang_switch": "EN",
        "viewport_group": "预览视口",
        "mode_group": "评审模式",
        "tools_group": "批注工具",
        "close_dialog": "关闭对话框",
        "zoom_keys": "+ / - / 滚轮",
        "cancel_tool": "取消当前工具或关闭弹窗",
        "comment_label": "批注内容",
        "prototype_label": "待评审原型",
        "roam_prev": "上一个批注",
        "roam_next": "下一个批注",
        "roam_label": "上一处/下一处批注",
        "tool_select": "选择：点击画布添加 Pin",
        "tool_draw": "按住拖拽圈画涂鸦",
        "tool_hand": "抓手平移画布（Space 按住拖拽）",
        "undo_label": "撤销标注",
        "zoom_in_t": "放大",
        "zoom_out_t": "缩小",
        "drawer_toggle": "收起/展开批注抽屉",
        "criteria_title": "验收准则 (Spec Matrix)",
        "criteria_title_short": "验收准则",
        "criteria_count": "准则 {checked}/{total}",
        "criteria_empty": "无 spec 判据来源（独立预览时属正常；随 /design-io 运行会自动载入）",
        "criteria_toggle_title": "查看/隐藏验收准则（S）",
        "theme_toggle": "切换明暗主题",
        "shortcuts_open": "查看完整快捷键指南",
        "got_it": "知道了",
        "toast_mode_preview": "进入纯净预览模式",
        "toast_mode_annotate": "进入批注评审模式",
        "toast_pin_added": "已添加批注 #{n}",
        "toast_loop_done": "圈画完成！输入修改意见后按 Enter 保存",
        "toast_resolved": "批注 #{n} 已标记为已解决",
        "toast_reopened": "批注 #{n} 已重新打开",
        "toast_drawer_open": "展开批注抽屉",
        "toast_drawer_closed": "已收起批注抽屉（点击右侧把手或按 [ 展开）",
        "toast_lang": "已切换至中文模式",
        "toast_focus": "聚焦批注 #{n}",
        "toast_vp": "已切换视口宽度",
        "confirm_desc": "确认本轮通过并进入实现（Fill）；需有批注或整体意见",
        "revise": "需要修改",
        "revise_desc": "打回修改并进入下一轮原型",
    },
    EN: {
        "round_n": "Round {n}",
        "annotate": "Annotate",
        "ready": "Ready",
        "drawer_title": "Annotations & confirm",
        "drawer_empty_title": "No annotations yet",
        "drawer_empty_desc": "Click any element on the canvas to annotate in place, press B for a box, or D to draw; overall feedback lives below",
        "collapse": "Collapse",
        "duplicate_anchor": "Already annotated as #{n}; not added again",
        "coachmark_title": "Start annotating",
        "coachmark_text": "Click any canvas element to annotate, Ctrl+↵ to approve (press ? for shortcuts)",
        "onboard_close": "Got it",
        "anchor_num_pre": "Anchor ",
        "anchor_num_post": " revision note",
        "anchor_placeholder": "What to change here",
        "remove_num_pre": "Remove anchor ",
        "field_label": "Overall feedback (optional)",
        "field_hint": "Pick an element on the canvas first, or write overall feedback below",
        "field_placeholder": "Overall notes on page structure and style; for a specific element, click it on the canvas",
        "close": "Close",
        "cancel": "Cancel",
        "terminate": "Abort preview",
        "terminate_confirm": "Abort this preview session?",
        "terminate_confirm_go": "Confirm abort",
        "terminate_desc": "Stops the whole preview session (not pass/fail); confirm again in the popover",
        "abort_cancelled": "Abort cancelled",
        "abort_popover_aria": "Confirm abort preview",
        "draft": "Keep notes, decide later",
        "draft_desc": "Close drawer and keep notes; do not submit a decision yet",
        "remove": "Remove",
        "anchor_note_label": "Anchor notes ({n}):",
        "no_text": "(no text)",
        "done_title": "Recorded",
        "done_body": "This window will close shortly.",
        "confirm": "Approve",
        "skip": "Skip",
        "skip_desc": "Skip this review without confirming approval (Shift+Esc)",
        "zoom_fit": "Fit",
        "draw_toggle": "Draw",
        "draw_on": "Drawing · click to stop",
        "draw_label": "Draw {n}",
        "tool_box": "Highlight box",
        "tool_ruler": "Ruler and spacing probe",
        "box_label": "Box {n}",
        "ruler_size": "{w}×{h} px",
        "ruler_distance": "{d} px",
        "app_title": "Preview & Confirm",
        "mode_preview": "Preview",
        "mode_annotate": "Annotate",
        "vp_desktop": "Desktop [1024px]",
        "vp_tablet": "Tablet [768px]",
        "vp_mobile": "Mobile [375px]",
        "filter_all": "All",
        "filter_pending": "Pending",
        "filter_resolved": "Resolved",
        "mark_resolved": "Mark as Resolved",
        "reopen": "Reopen",
        "tag_copy": "Copy",
        "tag_layout": "Layout",
        "tag_visual": "Visual",
        "tag_draw": "Draw",
        "tag_box": "Box",
        "tag_pin": "Pin",
        "tag_note": "Note",
        "tag_element": "Element",
        "popover_placeholder": "Type a revision note (Enter to save · Esc to cancel)",
        "popover_save": "Save",
        "popover_aria": "Inline annotation input",
        "tab_annotations": "Annotations",
        "tab_spec": "Criteria",
        "approve_ready": "Approve ({n} notes)",
        "approve_ready_one": "Approve (1 note)",
        "approve_not_ready": "Write a note or annotate to approve",
        "status_ready": "{n} annotations · Ready to approve",
        "status_not_ready": "Add notes or pin to confirm",
        "shortcuts_title": "Keyboard Shortcuts",
        "group_global": "Global Actions",
        "group_tools": "Canvas & Tools",
        "lang_toggle": "Toggle ZH/EN",
        "lang_switch": "中文",
        "viewport_group": "Preview viewport",
        "mode_group": "Review mode",
        "tools_group": "Annotation tools",
        "close_dialog": "Close dialog",
        "zoom_keys": "+ / - / wheel",
        "cancel_tool": "Cancel tool or close dialog",
        "comment_label": "Annotation comment",
        "prototype_label": "Prototype under review",
        "roam_prev": "Previous annotation",
        "roam_next": "Next annotation",
        "roam_label": "Next / Prev Annotation",
        "tool_select": "Select: click canvas to drop a pin",
        "tool_draw": "Draw: drag to loop",
        "tool_hand": "Hand: pan canvas (Space+drag)",
        "undo_label": "Undo annotation",
        "zoom_in_t": "Zoom in",
        "zoom_out_t": "Zoom out",
        "drawer_toggle": "Toggle annotation drawer",
        "criteria_title": "Acceptance Criteria (Spec Matrix)",
        "criteria_title_short": "Criteria",
        "criteria_count": "Criteria {checked}/{total}",
        "criteria_empty": "No spec criteria source (normal for standalone previews; /design-io loads it automatically)",
        "criteria_toggle_title": "Show/hide acceptance criteria (S)",
        "theme_toggle": "Toggle light/dark theme",
        "shortcuts_open": "Shortcut guide",
        "got_it": "Got it",
        "toast_mode_preview": "Clean preview mode",
        "toast_mode_annotate": "Annotation mode",
        "toast_pin_added": "Added annotation #{n}",
        "toast_loop_done": "Loop drawn! Type a note and press Enter to save",
        "toast_resolved": "Note #{n} marked as resolved",
        "toast_reopened": "Note #{n} reopened",
        "toast_drawer_open": "Drawer opened",
        "toast_drawer_closed": "Drawer collapsed (click the right tab or press [ )",
        "toast_lang": "Switched to English",
        "toast_focus": "Focused note #{n}",
        "toast_vp": "Viewport width switched",
        "confirm_desc": "Approve this round and proceed to Fill; needs notes or annotations",
        "revise": "Needs changes",
        "revise_desc": "Send back for revision and open the next prototype round",
    },
}


def lang() -> str:
    """Active locale (zh-CN or en). Env DPB_PREVIEW_LANG, then LANG, default zh-CN."""
    return resolve_ui_locale()


def t(key: str, **kw: object) -> str:
    """Translate a key in the active locale, with {kw} interpolation."""
    table = _STRINGS[lang()]
    val = table.get(key)
    if val is None:
        return key
    if kw:
        try:
            return val.format(**kw)
        except (KeyError, IndexError):
            return val
    return val


def default_options() -> list[str]:
    """[confirm_label, revise_label] in the active locale."""
    return [t("confirm"), t("revise")]


# Union of skip labels across ALL locales - a skip is an explicit
# non-confirm disposition (ADR-0008): it never enters CONFIRM_LABELS and
# can never satisfy the confirm floor or G5.
SKIP_LABELS: set[str] = {
    _STRINGS[ZH]["skip"], _STRINGS[EN]["skip"],
    "skip",
}

# Union of confirm labels across ALL locales. New CTA wording ("确认通过" /
# "Approve", T-083 REC-08) plus every historical label callers may have copied
# from older docs — old submitted values keep classifying as confirms.
CONFIRM_LABELS: set[str] = {
    _STRINGS[ZH]["confirm"], _STRINGS[EN]["confirm"],
    "确认签署决策", "Confirm & sign decision",
    "确认通过", "Confirm",
    "confirm", "confirmed", "pass", "ok",
}

# Union of revise labels across ALL locales - frontend must classify a revise
# regardless of which locale the UI rendered in.
REVISE_LABELS: set[str] = {
    _STRINGS[ZH]["revise"], _STRINGS[EN]["revise"],
    "revise", "needs changes", "needs-changes",
}
