...
def fork(
    ...
    fork_path = new_dir / "fork.json"
    if fork_path.is_file():
        raise VersionError(f"fork already exists: {fork_path}")
    record: dict[str, Any] = {
        "schema_version": VERSION_SCHEMA_VERSION,
        "branch": branch,
    ...
    _atomic_write(fork_path, _json_text(record))
    start = new_dir / "round-1.html"
    start.write_text(src["prototype_html"], encoding="utf-8")
