...
Six-block report (final state). Round 1: 1 条 R3 blocking（interaction 轨，dd: DD-0003 挑战方向假设）→ Recirculate → DD-0004 supersedes 修订 → Re-Fill → 失效集重采重评 → closure → Pass。基线漂移（W8）在终局前复核：DD-0003/0004 标 stale → 三出口「保持」（复核行重绑新 sha）。
